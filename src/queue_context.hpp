...
struct QueueContext : public IQueue<Value> {
  QueueContext(std::size_t max_size,
               std::shared_ptr<IOnOverflowStrategy<Value>> strategy,
               std::shared_ptr<IQueue<Value>> queue)
      : max_size_{max_size}, strategy_{strategy}, queue_{queue} {}
